...
prefix = 'ramp '; 
targetFiles = {'chrip-max.mat'};      % รายชื่อไฟล์ในโฟลเดอร์ (ไม่ต้องใส่ path)
% ตั้งค่าเวลาที่ต้องการเลื่อน (Manual Shift)
manual_shift_time = 0.011; 
...
% --- MATLAB Script: Manual Shift and 1-sec Trimming ---
for i = 1:numFiles
    filename = fileList{i};
    fullInputPath = fullfile(input_folder, filename); % สร้าง path เต็มเพื่อเรียกไฟล
    ...
    % ทำการเลื่อนแกนเวลา
    t_m_shifted = t_m - manual_shift_time;
    ...
    title(['Shifted (', num2str(manual_shift_time), 's) and Trimmed (Last 1s removed)']);
